...
# Route and Seat Selection Page
class Route_Seat_Page(tk.Frame):
    def __init__(self, parent, controller):
        super().__init__(parent, bg='darkblue')
        self.controller = controller
        self.selected_route = tk.StringVar()  # Initialize selected_route here
        self.create_widgets()
    ...
    def save_and_next(self):
        ...
        if not recline_seats and not bunk_seats:
            messagebox.showwarning("Input Error", "Please enter the number of seats.")
            return
